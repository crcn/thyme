...
var Broker = Structr({
	...
	'push': function(job, callback) {  
		...
		if(job._id) {
			this._queue.update({ _id: job._id }, { $set: { state: 'queued', data: job.data }}, callback);
	...
	'pop': function(count, callback) {   
		...
		//caught in the middle of popping? make it wait.
		if(this._popping) {                                       
		...
		step(
			...
			function() {
				self._queue.remove({ _id: runningJob._id }, function(){});
			}
		);
	},
