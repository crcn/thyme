...
exports.plugin = function(m)
{
	...
	function add(item, req)
	{
		...
		queue_c.findOne({ channel: channel, queueId: data._id }, function(err, cue)
		{
			//cue exists already
			if(cue) return;
	...
	function setInfo(item, req)
	{
		...
		console.success('Setting queue info for %s', item.channel );
		...
		info[ item.channel + toSet.cluster ] = toSet;
	}
