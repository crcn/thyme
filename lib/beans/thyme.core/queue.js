...
var Queue = Structr({
	...
	{
		if(this._nextTimeout) clearTimeout(this._nextTimeout);

		setTimeout(this.getMethod('next'), ms);
	},
	...
	'_call': function(item)
	{
		...
		// var route = this._mediator.getRoute('pull '+ item.channel, item.data, { meta: { cluster: item.cluster } }, next);
