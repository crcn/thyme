...
var Worker = Structr({
	...
	'run': function(skip)
	{
		...
		if((this._running && !skip) || !this._ops.channel || this._locked)
		{
			return;
		}          
	...
	{                                                                                    
		...
		//pull the request
		this._router.pull(this._ops.channel, job.data, { meta: { queue: this._target }}, function(response)
		{                          
	...
	'_sleep': function()
	{
		...
		this._queue.next(function(err, job)
		{
			...
			if(job)
			{
				var timeout = Math.max(job.sendAt - Date.now(), 500);
	...
	{        
		...
		{
			console.log('Worker %s:%s has %d items in queue', self._target, self._ops.channel, num);
		});
	}
});
