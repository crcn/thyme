...
var Worker = Structr({
	...
	'_maxWorkers': function() {
		return this._workers.throttle().max || this._ops.max;
	},
	...
	'run': function() {  
		...
		if(this._runningJobs.length > this._maxWorkers()) return false;
	...
	'_run': function(job) {
		...
		this._popJob(outcome.success(function(task, callback) {     
			...
			job.run(task, function(err, response) {    
	...
	'_popJob': function(callback) {
		...
		var timeout = this._workers.throttle().timeout, self = this;
	...
	'_countQueued': function() { 
		...
		this._.sendAtqueue.countQueued(Date.now(), function(num) {
